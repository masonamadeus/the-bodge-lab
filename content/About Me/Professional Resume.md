---
uid: mason-resume
contentHash: 353c91c0
date: '2025-12-01T23:37:00.725Z'
---

# Mason Amadeus Washer 

**Creative Director, Broadcast Engineer, On-Air Talent, IT Manager, Content Creator** 

Winchester, Kentucky | Business@MasonAmadeus.com

___

## Key Points 

{% grid %}

* Co-Creator of *Digital Folklore*, winner of multiple Signal Awards for production quality and hosting.
* Creative Director @ 8th Layer Media since 2022.
* 8 years in terrestrial radio broadcasting, 3 years as sole assistant to Chief Engineer.
* Expert in audio post production; highly proficient in other creative software including video editing and 3D. 
* 1 Year as Regional IT Manager for six separate clusters throughout New England; sysadmin, inventory/logistics, helpdesk, scripting, etc. 

``

* Creator of 3 NHAB Award-winning radio commercials.
* Deep knowledge of audio technology, industry standards, and production workflows. 
* Experience installing and maintaining AoIP (Dante & Wheatnet) and SIP (Telos) networks.
* Advanced understanding of Windows-based computer systems and AD environment. 
* Highly skilled in troubleshooting complex systems and creative problem solving. 
* Proficient with C# and Javascript. Knowledgeable in Powershell, Batch, Lua, CSS, HTML. 

{%endgrid%}

___

# Professional History 

## 8th Layer Media - Remote (July 2022 - Present)

#### Co-Creator / Executive Producer / Sound Designer for [Digital Folklore](https://digitalfolklore.fm) Podcast
* Achieved critical acclaim for [Digital Folklore](https://digitalfolklore.fm) with multiple Signal Awards, winning Gold for *Best Co-Host Team* and *Best Trailer*, Silver for *Best Indie Podcast*, and Bronze for *Most Innovative Audio Experience*.
* Co-created and launched an audio-drama hybrid podcast that analyzes internet culture through the lens of academic folklore.
* Engineered highly complex, immersive soundscapes; pioneered a unique format blending non-fiction interviews with narrative audio drama.
* Conducted extensive academic research and high-level interviews with folklore experts, synthesizing dense academic theory into accessible, entertaining narratives for a general audience.
* Managed end-to-end production workflows, from interviews and scriptwriting to final mastering and distribution logistics.

#### Other 8LM Projects and Shows
* Co-host and executive producer for The FAIK Files (on the [Cyberwire Network](https://thecyberwire.com/podcasts/the-faik-files)).
* Creator, editor, and host of [Wicked Interesting](/Video/Wicked%20Interesting/).

<br>

## Monadnock Broadcasting Group - Keene, NH (April 2014 - July 2022) 

#### Regional IT Manager (Promoted in June 2021) 
* Managed all IT requirements for six separate clusters in NH, MA, and VT. 
* Built/Installed/Maintained fileservers, HyperV Hosts, GPOs, VLANs, office machines, radio-specific software. 
* Solely developed a windows application called “NewsJock” for use in small market newsrooms. Has been adopted in several markets. 
* Supported critical live infrastructure with minimal downtime.
* Loved this role; only left so soon because of once-in-a-lifetime chance with 8LM!

#### Assistant Engineer (Promoted in June 2018) 
* Assisted C.E. in refitting our existing studios with new Wheatstone AoIP consoles and blades in 2018. 
* Aided in commissioning new transmitters and HD importer/exporter systems in 2019. 
* Acted as first point of contact to resolve major equipment failures including a loss of our main IPSTL relay point, tower site remote control failures, master antenna system combiner/filter failure, rebroadcast receiver signal path issues, etc. 
* Covered other engineers' duties as needed across multiple states and markets.
* Recapped & Rebuilt multiple small transmitters and transmitter components.

#### On-Air / Production (Original hired position, April 2014) 
* Hosted a weekday afternoon drive program on a Hot AC music station. 
* Filled in when necessary for other staff, including Production Director and morning show hosts. 
* Wrote and produced radio commercials which won 3 NHAB awards for “Best Individual Radio Commercial” in 2016, 2017, and 2020.

<br>

## DeMoulas Market Basket - Swanzey, NH (February 2011 - April 2014) 

#### Full-Time Grocery Associate 
* Tracked/Ordered a single aisle of a mid-sized grocery store. (Peanut butter and candy; the best aisle.)

<br>

___

## Software Expertise

* Mastery of REAPER, including advanced scripting and custom automation.
* Expert in Adobe Audition.
* Supported ProTools Studios professionally.
* Highly proficient with Blender 3D.
* Proficient with Davinci Resolve. 
* Proficient with Descript.
* Proficient with Affinity graphic design suite.

___

## Additional Skills 

* Voice Actor for various audiodrama podcasts including *Hemophobia*, *Inn Between*, *Something's Amiss at Juniper's House*, and more.
* Elaborate Live Streaming Event Designer. (Once built a face-spraying robot that raised over $700).
* Deep understanding of AI systems including transformer-based LLMs and diffusion models (I'm not an AI developer, though).
* Doesn't take self too seriously.
* Works well with others in high-intensity environments.
* Cheerful & Positive. 
* Calm Under Pressure. 
* Voracious Learner. 

