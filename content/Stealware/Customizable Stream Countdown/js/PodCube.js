...
const log = {
    info: (msg, data) => CONFIG.DEBUG && console.log(`[PodCube]`, msg, data || ""),
    warn: (msg, data) => console.warn(`[PodCube WARN]`, msg, data || ""),
    error: (msg, data) => console.error(`[PodCube ERROR]`, msg, data || "")
};
...
class FeedNormalizer {
    ...
    static analyzeTitle(rawTitle) {
        ...
        if (episodeType === EPISODE_TYPES.TRANSMISSION || episodeType === EPISODE_TYPES.PODCUBE_HQ) {
            ...
            cleanTitle = rawTitle.replace(EMOJI_PATTERNS.TWIBBIE_ONDEMAND, "").trim();
        }
    ...
    static normalizeItem(rawItem, format = 'rss') {
        try {
            ...
            const id = rawItem.id || rawItem.guid || null;
...
class PodCubeEngine {
    ...
    async init(force = false) {
        ...
        try {
            ...
            log.info(`Initializing PodCube with ${CONFIG.FEED_TYPE.toUpperCase()} feed:`, feedUrl);
        ...
        try {
            ...
            if (format === 'json') {
                ...
                const getText = (parent, tagName) => {
                    //  Try finding the tag by name (handles 'itunes:duration' or 'duration')
                    const tags = parent.getElementsByTagName(tagName);
                    if (tags.length > 0) return tags[0].textContent;

                    //  If it has a colon, try looking for just the local name
                    if (tagName.includes(':')) {
                        const localName = tagName.split(':')[1];
                        const localTags = parent.getElementsByTagName(localName);
                        if (localTags.length > 0) return localTags[0].textContent;
                    }
                    return null;
                };
            ...
            this.episodes = items
                .map(item => {
                    ...
                    return normalized ? new Episode(normalized) : null;
                })
                .filter(ep => ep !== null);
    ...
    get status() {
        ...
        return {
            ...
            remaining: Math.max(0, (this._audio.duration || 0) - this._audio.currentTime),
            percent: percent,
    ...
    get queueDuration() {
        // Calculate total remaining time (current track remainder + future tracks)
        if (this._queue.length === 0) return 0;

        let total = 0;

        //  Add remaining time of current track
        if (this.status.playing && this.status.duration) {
            total += (this.status.duration - this.status.time);
        } else if (this._queue[this._queueIndex]) {
            total += this._queue[this._queueIndex].duration || 0;
        }
 ...
 */
     ...
     * Returns episodes with same model, origin, or tags
     */
    findRelated(episode, limit = 10) {
        ...
        const scored = this.episodes
            .filter(e => e !== episode) // Don't match self
            .map(e => {
                let score = 0;
                if (e.model === episode.model) score += 3;
                if (e.origin === episode.origin) score += 2;
                if (e.tags) {
                    // Count shared tags
                    e.tags.forEach(t => { if (targetTags.has(t)) score += 1; });
                }
                return { ep: e, score };
            })
